...
async def main():
    ...
    mode_group.add_argument("--hubs", action="store_true", help="Show EVO hubs for each organization")
    mode_group.add_argument("--workspaces", action="store_true",
                        help="Show workspaces for a specific organization (requires --org-id and --hub-url)")
    mode_group.add_argument("--files", action="store_true", help="Show files for a specific workspace (requires --workspace-id and --hub-url -o)")
    mode_group.add_argument("--objects", action="store_true", help="Show objects for a specific workspace (requires --workspace-id and --hub-url -o)")

    parser.add_argument("--org-id", help="Organization ID (required with --workspaces)")
    parser.add_argument("--hub-url", help="hub URL (required with --workspaces)")
    ...
    # Validate arguments
    if args.workspaces and (not args.org_id or not args.hub_url):
        parser.error("--workspaces requires both --org-id and --hub-url")

    if args.files and (not args.workspace_id or not args.hub_url or not args.org_id):
        parser.error("--files requires both --workspace-id, --hub-url and --org-id")

    if args.objects and (not args.workspace_id or not args.hub_url or not args.org_id):
        parser.error("--objects requires both --workspace-id, --hub-url and --org-id")
    ...
    if args.workspaces:
        auth_data = await login(args.client_id)
        workspaces = await get_workspace_page(auth_data, args.org_id, args.hub_url)
        print_workspaces(workspaces)

    if args.files:
        auth_data = await login(args.client_id)
        environment = Environment(hub_url=args.hub_url, org_id=args.org_id, workspace_id=args.workspace_id)
